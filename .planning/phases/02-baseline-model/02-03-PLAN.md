...
<objective>
Implement the complete Trainer class with training loop, validation, checkpointing, TensorBoard logging, early stopping, and learning rate scheduling.

Purpose: The trainer orchestrates model training with all the infrastructure needed for convergent training, monitoring, and reproducibility. Without this, we cannot train the baseline model.
...
<task type="auto">
  <name>Task 1: Implement Trainer initialization and core loops</name>
  <files>src/training/trainer.py</files>
  <action>
Implement the Trainer class __init__, train_epoch(), and validate() methods in trainer.py.
