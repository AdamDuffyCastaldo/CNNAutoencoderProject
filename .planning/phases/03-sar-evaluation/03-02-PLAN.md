...
must_haves:
  truths:
    ...
    - "Compression ratio calibration achieves target within 20% tolerance"
...
2. Implement `JPEG2000Codec(Codec)`:
   ...
   - Quality parameter: IMWRITE_JPEG2000_COMPRESSION_X1000 (range 1-1000)
   - Lower value = more compression
   - Convert input [0,1] float to uint8 for encoding, back to float32 for decoding
   - Reference RESEARCH.md Pattern 3 for implementation

3. Add compression ratio calibration:
   - Binary search for quality parameter matching target ratio
   - Cache calibrated parameters (dict mapping target_ratio -> quality_param)
   - Tolerance check: abs(achieved - target) / target < tolerance
