...
must_haves:
  truths:
    ...
    - "scripts/evaluate_model.py calls Evaluator.save_results and produces JSON files in evaluations/ directory"
  artifacts:
  ...
  key_links:
    ...
    - from: "scripts/evaluate_model.py"
      to: "evaluations/"
      via: "Evaluator.save_results call"
      pattern: "evaluator\\.save_results.*output"
---
...
2. Update `evaluate_batch` to use compute_all_metrics for consistency:
   - REPLACE manual metric computation calls with single compute_all_metrics() call:
     ```python
     def evaluate_batch(self, batch: torch.Tensor) -> Tuple[torch.Tensor, dict]:
         # ... forward pass to get x_hat ...

         # For each sample, compute all metrics using the unified function
         for i in range(batch_size):
             orig_np = batch[i, 0].cpu().numpy()
             recon_np = x_hat[i, 0].cpu().numpy()

             # Single call replaces manual psnr, ssim, epi, etc. calls
             sample_metrics = compute_all_metrics(orig_np, recon_np)
             # sample_metrics contains: mse, psnr, ssim, ms_ssim, epi,
             #                          enl_ratio (dict), histogram (dict), etc.

             # Aggregate into batch metrics
             ...
     ```
   - This ensures evaluate_batch uses same metric implementations as codec evaluation
   - Handle edge cases (NaN values, small images) gracefully
...
3. Main evaluation flow:
   ...
   def main(args):
       ...
       # Save results - CRITICAL: This produces JSON in evaluations/
       output_dir = Path(args.output)
       output_dir.mkdir(parents=True, exist_ok=True)
       evaluator.save_results(results, output_dir, model_name)
       # ^ This call writes {model_name}_eval.json and {model_name}_detailed.json
       ...
       if args.compare_codecs:
           rd_data = []  # Collect rate-distortion points

           # Add autoencoder point
           rd_data.append({
               'name': model_name,
               'bpp': compute_bpp(original_shape, latent_bits),
               'psnr': results['psnr']['mean'],
               'ssim': results['ssim']['mean']
           })

           # Evaluate each codec at target compression ratios
           for codec_class in [JPEG2000Codec, JPEGCodec]:
               codec = codec_class()
               codec_evaluator = CodecEvaluator(codec)

               for target_ratio in args.compression_ratios:
                   codec_evaluator.calibrate([target_ratio], sample_images)
                   codec_results = codec_evaluator.evaluate_batch(test_images, target_ratio)

                   # MERGE codec results into rd_data with explicit format
                   rd_data.append({
                       'name': codec.name,
                       'compression_ratio': target_ratio,
                       'bpp': 32.0 / target_ratio,  # bits_per_pixel = 32 / compression_ratio
                       'psnr': codec_results['metrics']['psnr']['mean'],
                       'ssim': codec_results['metrics']['ssim']['mean'],
                       'achieved_ratio': codec_results['achieved_ratio']
                   })

       # Generate rate-distortion plot with merged data
       visualizer.plot_rate_distortion(rd_data, output_dir / 'rate_distortion.png')

       # Save rd_data to CSV for further analysis
       pd.DataFrame(rd_data).to_csv(output_dir / 'rate_distortion.csv', index=False)
...
```
  ...
  <done>
scripts/evaluate_model.py runs with --help. Script loads checkpoint, evaluates on test data, saves JSON results via Evaluator.save_results, generates visualizations. Codec results explicitly merged into rd_data list with name, bpp, psnr, ssim keys. Output structure matches specification.
  </done>
</task>
...
6. evaluations/{model_name}/ directory contains _eval.json and _detailed.json files
</verification>
...
6. Codec results merged into rd_data list when --compare-codecs is used
</success_criteria>
