...
<architecture_notes>
## Key Differences from Existing ResNet-Lite (ResidualBlock)

The existing `ResidualBlock` in blocks.py uses **post-activation** (ResNet v1):
```
...
The new `PreActResidualBlock` uses **pre-activation** (ResNet v2):
```
x → BN → ReLU → Conv → BN → ReLU → Conv → (+x)
    ↑                                      ↑
    ReLU BEFORE convs              NO ReLU after addition
```

**Why this matters:**
- Pre-activation provides cleaner gradient flow through the identity path
- The skip connection is truly identity (no activation blocking it)
- Empirically shows 0.3-1.0 dB improvement in deeper networks
- Reference: He et al. "Identity Mappings in Deep Residual Networks" (2016)

## Code Reuse Notes

- **Keep existing blocks**: `ResidualBlock`, `ResidualBlockWithDownsample`, `ResidualBlockWithUpsample` remain unchanged (used by ResNet-Lite)
- **Implement CBAM stubs**: `ChannelAttention`, `SpatialAttention`, `CBAM` already exist as stubs in blocks.py - implement them in place
- **Add new PreAct blocks**: These are new classes, not replacements for existing ones
</architecture_notes>
...
<task type="auto">
  <name>Task 2: Implement CBAM Attention Module (Complete Existing Stubs)</name>
  <files>src/models/blocks.py</files>
  <action>
**IMPORTANT:** ChannelAttention, SpatialAttention, and CBAM classes already exist as stubs in blocks.py (lines 295-386).
Replace the `raise NotImplementedError` with working implementations - do NOT create duplicate classes.
...
**Note:** Update `test_blocks()` function to include tests for the new PreActResidualBlock and CBAM implementations.
</verification>
