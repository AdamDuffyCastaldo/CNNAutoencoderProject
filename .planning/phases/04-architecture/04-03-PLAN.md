...
CBAM (Convolutional Block Attention Module) applies two types of attention:

1. **Channel Attention**: "Which feature channels are most important?"
   - Squeeze spatial dimensions → MLP → expand back
   - Learns to weight channels by their relevance

2. **Spatial Attention**: "Which spatial locations matter most?"
   - Pool across channels → Conv → expand
   - Learns to focus on important regions (edges, structures)

Applied after every residual block = 16 CBAM modules total (8 encoder + 8 decoder).

## Comparison with Variant B (ResidualAutoencoder)

| Aspect | Variant B (Residual) | Variant C (Attention) |
|--------|---------------------|----------------------|
| Residual blocks | PreActResidualBlock | PreActResidualBlock + CBAM |
| Total blocks | 16 (8 enc + 8 dec) | 16 residual + 16 CBAM |
| Parameters | ~5-7M | ~6-8M (slight increase) |
| Memory | Baseline | +10-20% overhead |
| Target improvement | +1.5 dB over baseline | +0.5 dB over Variant B |

## Code Reuse Strategy

**Reuse from ResidualAutoencoder (Plan 02):**
- Same overall encoder/decoder structure
- Same stem and output layers
- Same helper methods interface
- Same `_init_weights()` function

**Key difference:** Replace `PreActResidualBlock` with `ResidualBlockWithCBAM` wrapper:
```python
...
This keeps the architecture modular - just swap the block type.
</architecture_notes>
